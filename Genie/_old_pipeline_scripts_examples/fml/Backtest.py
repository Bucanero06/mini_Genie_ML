...
def __add_labels(data_file_names, data_file_dirs):
    # tick_bars = pd.read_csv(backtest_file_path, index_col=0, parse_dates=True)
    tick_bars = Data_Manager().fetch_csv_data_dask(data_file_name=data_file_names, search_in=data_file_dirs)
...
def backtest_cv(data_file_names, data_file_dirs):
    # tick_bars = pd.read_csv(backtest_file_path, index_col=0, parse_dates=True)
    tick_bars = Data_Manager().fetch_csv_data_dask(data_file_name=data_file_names, search_in=data_file_dirs)
    tick_bars = tick_bars.iloc[-1000:]
    tick_bars = tick_bars.dropna()
    ask = tick_bars['ask'].copy()
    bid = tick_bars['bid'].copy()
    closing = (ask + bid) / 2
