...
def get_weights_and_avgu(close_price, df, threads, times):
    ...
    numCoEvents = mlf.util.multiprocess.mp_pandas_obj(mpNumCoEvents, ('molecule', df.index),
                                                        threads, closeIdx=close_price.index, t1=times)
    numCoEvents = numCoEvents.loc[~numCoEvents.index.duplicated(keep='last')]
    numCoEvents = numCoEvents.reindex(close_price.index).fillna(0)
    out = pd.DataFrame()
    out['tW'] = mlf.util.multiprocess.mp_pandas_obj(mpSampleTW, ('molecule', df.index),
                                                      threads, t1=times, numCoEvents=numCoEvents)

    out['w'] = mlf.util.multiprocess.mp_pandas_obj(mpSampleW, ('molecule', df.index), threads,
