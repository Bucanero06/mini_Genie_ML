...
get_av_uniqueness_from_triple_barrier function from the mlfinlab_src package.
...
.. py:currentmodule:: mlfinlab_src.sampling.concurrent
...
.. code-block::
   ...
   import pandas as pd
   import numpy as np
   from mlfinlab_src.sampling.concurrent import get_av_uniqueness_from_triple_barrier
...
The core functions behind Sequential Bootstrapping are implemented in mlfinlab_src and can be seen below:

.. py:currentmodule:: mlfinlab_src.sampling.bootstrapping
...
One can use get_ind_matrix method from mlfinlab_src to build indicator matrix from triple-barrier events.

.. code-block::

   triple_barrier_ind_mat = get_ind_matrix(barrier_events)

We can get average label uniqueness on indicator matrix using get_ind_mat_average_uniqueness function from mlfinlab_src.
...
To be as close to the mlfinlab_src implementation let's convert ind_mat to numpy matrix
...
Let's see how this example is solved by the mlfinlab_src implementation. To reproduce that:
...
mlfinlab_src supports two methods of applying sample weights. The first is weighting an observation based on its given return
...
.. py:currentmodule:: mlfinlab_src.sample_weights.attribution
...
.. code-block::
    ...
    import pandas as pd
    import numpy as np
    from mlfinlab_src.sample_weights.attribution import get_weights_by_return
...
.. code-block::
    ...
    import pandas as pd
    import numpy as np
    from mlfinlab_src.sample_weights.attribution import get_weights_by_time_decay
